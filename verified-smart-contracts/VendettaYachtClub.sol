...
contract VendettaYachtClub is ERC721Enumerable, Ownable {
    // declaring required variables and they are understandble by the name
    using Strings for uint256;
    string NFT_URI;
    string public extension = ".json";
    string public hidden_URI;
    uint256 public cost = 0.075 ether; // Cost per NFT
    uint32 public maximum_supply = 10000; // Max number of NFTs
    uint32 public maximum_mint = 20; // Max mint amount per transaction
    bool public pause = false; // Paused or not
    ...
    function mint(uint256 _mintcount) public payable {
        ...
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintcount, 
                "Not enough ETH sent. Please check the price"
            );
        }
    ...
    //Function to withdraw the balance in smart contract to the owner wallet
