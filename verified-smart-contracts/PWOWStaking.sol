...
/* STAKING MECHANICS */
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused {
      ...
      if (reward > 0) {
        IPWOW PWOW = IPWOW(erc20Address);
        PWOW.mint(msg.sender, reward);
      }
    }
    ...
    function deposit(address account,uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
               ...
               if (msg.sender != erc721Address) {
                // dont do this step if its a mint + stake
                require(IERC721(erc721Address).ownerOf(tokenIds[i]) == msg.sender, "This NFT does not belong to address");
