...
contract Ztest is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  address ZtestWallet = 0xf225aEB628eC44FDA4EfEE43c092C33Dd4254F3D;
    //這邊之後會換成會使用溫錢包地址
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    payable(ZtestWallet).transfer(msg.value);
  ...
  function settestWallet(address _testWallet) external onlyOwner {
    ZtestWallet = _testWallet;
  }

  function withdraw() external payable onlyOwner {
    payable(ZtestWallet).transfer(address(this).balance);
  }
