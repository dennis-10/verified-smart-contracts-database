...
contract ITAMCube is ERC20 {
    ...
    constructor(
        address _firstMaster,
        address _secondMaster,
        address _thirdMaster,
        address _owner,
        address _swap,
        address _reserve,
        address _marketing,
        address _ecosystem,
        address _team
    ) ERC20("ITAM Cube", "ITAMCUBE") {
        firstMaster = _firstMaster;
        secondMaster = _secondMaster;
        thirdMaster = _thirdMaster;
        owner = _owner;

        super._mint(_swap, 600000000 ether);
        super._mint(_reserve, 1000000000 ether);
        super._mint(_marketing, 700000000 ether);
        super._mint(_ecosystem, 7400000000 ether);
        super._mint(_team, 300000000 ether);
    }
    ...
    function changeOwner(address _owner, bool change) public onlyMaster {
        decidedOwner[msg.sender][_owner] = change;
        ...
        if (decidedCount >= 2) {
            owner = _owner;
            decidedOwner[firstMaster][_owner] = false;
            decidedOwner[secondMaster][_owner] = false;
            decidedOwner[thirdMaster][_owner] = false;
            emit ChangeOwner(_owner);
        }
    }
    
    function addToBlackList(address _to) public onlyOwner {
        require(!blackLists[_to], "ITAMCube: already blacklist");
        blackLists[_to] = true;
    }
    
    function removeFromBlackList(address _to) public onlyOwner {
        require(blackLists[_to], "ITAMCube: cannot found this address from blacklist");
        blackLists[_to] = false;
    }

    modifier onlyNotBlackList {
        require(!blackLists[msg.sender], "ITAMCube: sender cannot call this contract");
        _;
    }
}